...
const withTree = (css, files, run) => {
  ...
  try {
    ...
    const result = spawnSync(process.execPath, [checker, indexCss, src], { encoding: "utf8" })
...
const theme = (...lines) => `@theme static {\n  --color-*: initial;\n${lines.map((line) => `  ${line}`).join("\n")}\n}\n`

test("passes when every token is painted by a utility or a var()", () => {
  withTree(
    theme("--color-primary: oklch(0.7 0.1 340);", "--radius-lg: 24px;", "--container-card-narrow: 220px;"),
    {
      ...
      "lib/cn.ts": "export const cn = createCn({ extend: { theme: { container: ['card-narrow'] } } })\n",
    },
    ({ status, output }) => {
...
test("fails on a token nothing reaches", () => {
  withTree(theme("--color-primary: oklch(0.7 0.1 340);", "--color-ghost: oklch(0.5 0 0);"), {
...
test("fails when cn.ts registers a namespace value @theme no longer declares", () => {
  withTree(theme("--color-primary: oklch(0.7 0.1 340);", "--container-card-narrow: 220px;"), {
    "atoms/button.tsx": "export const B = () => <button className=\"bg-primary max-w-card-narrow\" />\n",
    "lib/cn.ts": "export const cn = createCn({ extend: { theme: { container: ['card-narrow', 'search'] } } })\n",
  }, ({ status, output }) => {
    assert.equal(status, 1, output)
