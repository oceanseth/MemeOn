...
const split = (token) => {
  ...
  const namespace = NAMESPACES.filter((ns) => bare === ns || bare.startsWith(`${ns}-`)).sort(
...
const reaches = (token) => {
  ...
  if (namespace === 'breakpoint')
    return new RegExp(`(?<![\\w-])(max-)?${escape(name)}:`).test(classes)
  ...
  const probe = new RegExp(
    `(?<![\\w-])(${prefixes.map(escape).join('|')})-${escape(name)}(?![\\w-])`,
  )
