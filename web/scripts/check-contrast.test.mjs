...
const stylesheet = (overrides = {}) => {
  ...
  const tokens = {
    ...
    '--color-secondary': 'light-dark(oklch(98.5% 0.006 285), oklch(27% 0.03 285))',
    ...
    '--color-tier-holo-chip':
      'light-dark(var(--ramp-ultraviolet-200), var(--ramp-ultraviolet-800))',
    '--color-tier-holo-chip-text':
      'light-dark(var(--ramp-ultraviolet-900), var(--ramp-ultraviolet-100))',
    ...
    '--color-tier-prismatic-chip':
      'light-dark(var(--ramp-bubblegum-200), var(--ramp-bubblegum-800))',
...
test('checks each arm of a light-dark() pair on its own and names the arm that fails', () => {
  ...
  const dim = {
    '--color-muted-foreground': 'light-dark(oklch(45% 0.03 285), oklch(60% 0.03 285))',
  }
  ...
  withStylesheet(stylesheet(dim), (result) => {
    ...
    assert.match(
      result.output,
      /--color-muted-foreground on --color-background \(dark\) is Lc -\d+\.\d/,
    )
    assert.match(
      result.output,
      /--color-muted-foreground on --color-accent \(dark\) is Lc -\d+\.\d/,
    )
    // the light arm of the same token still passes, and still prints
    assert.match(
      result.output,
      /--color-muted-foreground\s+on\s+--color-background\s+light\s+Lc\s+\d+\.\d\s+ok/,
    )
    // Paper's chip label is the same muted ink on the pressed surface, so it fails with it
    assert.match(
      result.output,
      /--color-tier-paper-chip-text on --color-tier-paper-chip \(dark\) is Lc/,
    )
...
test('follows var() into a light-dark() and lands on a different arm value each side', () => {
  withStylesheet(stylesheet(), (result) => {
    ...
    const dark = result.output.match(
      /--color-link\s+on\s+--color-background\s+dark\s+Lc\s+(-?\d+\.\d)/,
    )
...
test("fails an ink no pair reads, even where the token is nobody's background", () => {
  const markup = {
    'Rail.css': '  .rail-note { color: var(--color-border); }\n',
  }
...
test('passes a glyph stroked in an audited ink, on either surface a podium tile paints', () => {
  ...
  withStylesheet(
    ...
    (result) => {
      ...
      const row = (fg, bg, arm, sign) =>
        new RegExp(`${fg}\\s+on\\s+${bg}\\s+${arm}\\s+Lc\\s+${sign}\\d+\\.\\d\\s+ok`)
