...
const runChecker = (files) => {
  ...
  try {
    ...
    const result = spawnSync(process.execPath, [checker, dist], {
      encoding: 'utf8',
    })
...
test('rejects a bundle whose first declaration is a bare components block', () => {
  const result = runChecker({
    'assets/index-abc123.css':
      '@layer components{@keyframes atom-rot{to{transform:rotate(360deg)}}}' + RESPELLED,
  })
...
test('rejects an order statement that inverts base and components, or drops a layer', () => {
  const inverted = runChecker({
    'assets/index-abc123.css': '@layer theme,components,base,utilities;' + BODY,
  })
  const incomplete = runChecker({
    'assets/index-abc123.css': '@layer theme,base,utilities;' + BODY,
  })
...
test("rejects properties ranked above theme, where Tailwind's @property fallbacks would beat utilities", () => {
  const result = runChecker({
    'assets/index-abc123.css':
      '@layer theme,base,components,utilities;' + BODY + '@layer properties{*{--tw-x:0}}',
  })

  assert.equal(result.status, 1, result.output)
  assert.match(
    result.output,
    /@property fallbacks live in "properties" and have to rank below "theme"/,
  )
})
...
test('reads through comments and strings rather than trusting the first @layer-looking text', () => {
  const result = runChecker({
    'assets/index-abc123.css':
      '/* @layer components { */.a:before{content:"@layer components {"}' + ORDER + BODY,
  })
...
test('exits 2 when there is nothing built to check', () => {
  const missing = spawnSync(
    process.execPath,
    [checker, join(tmpdir(), 'memeon-layer-check-absent')],
    {
      encoding: 'utf8',
    },
  )
