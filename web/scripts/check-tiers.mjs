...
const hookProblem = (name) =>
  CONTEXT_API.has(name)
    ? 'React context below views/ (only an atom may create or read a variant context)'
    : 'React state hook below views/ (lift it into hooks/ and pass a prop)'
...
const ALIAS = '@/'
const STATE_LIBS =
  /^(mobx|mobx-react(-lite)?|zustand|jotai|valtio|recoil|redux|@reduxjs\/toolkit|react-redux|@tanstack\/react-query|swr|@xstate\/react|xstate)(\/|$)/
...
const relativeModuleFile = (fromFile, spec) => {
  ...
  const candidates = [
    ...
    ...['index.ts', 'index.tsx', 'index.js', 'index.jsx'].map((index) => join(base, index)),
  ]
    ...
    if (target === 'react' && statement.exportClause && ts.isNamedExports(statement.exportClause)) {
      return statement.exportClause.elements.some(
        (element) =>
          ...
          CHECKED.has(element.propertyName?.text ?? element.name.text),
      )
    }
...
const reportModule = (file, tier, spec) => {
  ...
  if (folder === COPY) {
    report(
      file,
      `${tier} imports from copy/ ("${spec}"): strings reach components through the model`,
    )
    return
  }
  if (
    folder !== undefined &&
    (TIERS.includes(folder) || ENGINES.includes(folder)) &&
    !ALLOWED[tier].includes(folder)
  ) {
...
const inspectCopySource = (file) => {
  ...
  const check = (spec) => {
    ...
    report(
      file,
      `copy/ imports "${spec}": copy is plain data and imports only copy/ (plus ${[...COPY_HELPERS].join(', ')})`,
    )
  }
    ...
    if (ts.isImportDeclaration(statement)) {
      ...
      if (bindings && ts.isNamedImports(bindings)) {
        for (const binding of bindings.elements) {
          ...
          if (TREE_NAMES.has(importedName))
            report(file, `value import of ${importedName} from ${spec}`)
        }
      }
      continue
    }
    ...
    if (
      ...
      if (
        ...
        for (const element of statement.exportClause.elements) {
          ...
          if (TREE_NAMES.has(exportedName))
            report(file, `value import of ${exportedName} from ${spec}`)
        }
      }
    }
  }
...
const reactDeclaration = (declaration) =>
  /node_modules[\\/](@types[\\/]react|react)[\\/]/.test(declaration.getSourceFile().fileName)
...
const isReactHookSymbol = (symbol) =>
  symbol && CHECKED.has(symbol.getName()) && symbol.declarations?.some(reactDeclaration)

const isReactNamespaceSymbol = (symbol) =>
...
const reactDestructuredHookName = (checker, symbol) => {
  for (const declaration of symbol?.declarations ?? []) {
    ...
    if (
      ...
      isReactNamespaceSymbol(checker.getSymbolAtLocation(variable.initializer))
    )
      return propertyName
  }
  return undefined
...
const bindingPatternContains = (name, target) => {
  ...
  if (ts.isObjectBindingPattern(name) || ts.isArrayBindingPattern(name))
    return name.elements.some(
      (element) => ts.isBindingElement(element) && bindingPatternContains(element.name, target),
    )
  return false
}
...
const isShadowed = (node, name) => {
  for (let current = node.parent; current && !ts.isSourceFile(current); current = current.parent) {
    if (
      ...
      current.parameters.some((parameter) => bindingPatternContains(parameter.name, name))
    )
      return true
    if (
      ts.isBlock(current) &&
      current.statements.some((statement) => statementDeclares(statement, name))
    )
      return true
    if (
      ts.isCatchClause(current) &&
      current.variableDeclaration &&
      bindingPatternContains(current.variableDeclaration.name, name)
    )
      return true
    if (
...
const inspectTierSource = (file, tier, checker, program) => {
  ...
  for (const statement of sourceFile.statements) {
    if (ts.isImportDeclaration(statement)) {
      const spec = ts.isStringLiteral(statement.moduleSpecifier)
        ? statement.moduleSpecifier.text
        : undefined
      if (!spec || isTypeOnlyImport(statement.importClause)) continue
      reportModule(file, tier, spec)
      if (statement.importClause) {
        if (spec === 'react' && statement.importClause.name)
          reactNamespaceImports.add(statement.importClause.name.text)
        const bindings = statement.importClause.namedBindings
        if (spec === 'react' && bindings && ts.isNamespaceImport(bindings))
          reactNamespaceImports.add(bindings.name.text)
        if (bindings && ts.isNamedImports(bindings)) {
          for (const binding of bindings.elements) {
            if (binding.isTypeOnly) continue
            const importedName = binding.propertyName?.text ?? binding.name.text
            if (
              CHECKED.has(importedName) &&
              (spec === 'react' || isReactHookExport(file, spec, importedName))
            )
              stateHookImports.set(binding.name.text, importedName)
            if (spec === 'react' && importedName === 'default')
              reactNamespaceImports.add(binding.name.text)
          }
        }
      }
      continue
    ...
    if (
      ...
      if (ts.isStringLiteral(statement.moduleSpecifier))
        reportModule(file, tier, statement.moduleSpecifier.text)
    }
  }
    ...
    if (ts.isCallExpression(node)) {
      ...
      if (tier !== 'views') {
        ...
        const direct =
          ...
          stateHookImports.has(callee.text) &&
          !isShadowed(callee, callee.text)
            ? stateHookImports.get(callee.text)
            : undefined
        const namespaced =
          ts.isPropertyAccessExpression(callee) &&
          ts.isIdentifier(callee.expression) &&
          reactNamespaceImports.has(callee.expression.text) &&
          CHECKED.has(callee.name.text) &&
          !isShadowed(callee.expression, callee.expression.text)
            ? callee.name.text
            : undefined
...
if (problems.length > 0) {
  console.error(
    `check-tiers: ${problems.length} problem(s) (${summary})\n` +
      problems
        .sort()
        .map((problem) => `  ${problem}`)
        .join('\n'),
