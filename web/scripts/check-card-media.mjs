...
if (!sawCardMedia) {
  findings.unshift(
    '  lib/cardMedia.ts is missing — it owns the shared observer and --glow-play-state',
  )
}
...
if (findings.length > 0) {
  console.error(
    `check-card-media: ${findings.length} location(s) break the card-media owner contract\n${findings.join('\n')}`,
  )
  process.exit(1)
}
