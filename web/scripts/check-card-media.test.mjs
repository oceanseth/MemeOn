...
test("skips tests and stories that mock IntersectionObserver", () => {
  withSrc({
    ...LEGAL,
    "lib/cardMedia.runtime.test.tsx": "class Fake implements IntersectionObserver { observe() { new IntersectionObserver(() => {}) } }\n",
    "molecules/meme-card.stories.tsx": "export const Video = { play: () => new IntersectionObserver(() => {}) }\n",
    "hooks/market-regressions.runtime.test.tsx": "vi.stubGlobal('IntersectionObserver', class {})\n",
  }, ({ status, output }) => {
    assert.equal(status, 0, output)
  })
})
...
test("fails when cardMedia constructs IntersectionObserver instead of ViewportObserver", () => {
  withSrc({
    ...LEGAL,
    "lib/cardMedia.ts": CARD_MEDIA.replace("new ViewportObserver(() => {})", "new IntersectionObserver(() => {})"),
  }, ({ status, output }) => {
    assert.equal(status, 1, output)
    assert.match(output, /lib\/cardMedia\.ts:/)
...
test("fails when --glow-play-state mutation leaves lib/cardMedia.ts", () => {
  withSrc({
    ...LEGAL,
    "atoms/foil.ts": "export const pause = (el) => el.style.setProperty('--glow-play-state', 'paused')\n",
  }, ({ status, output }) => {
    assert.equal(status, 1, output)
...
test("fails when a meme-card component mentions IntersectionObserver or drops the model handles", () => {
  withSrc({
    ...LEGAL,
    "molecules/meme-card.tsx": "export function MemeCard() { new IntersectionObserver(() => {}); return <article /> }\n",
  }, ({ status, output }) => {
    assert.equal(status, 1, output)
