...
const writeMap = (file, sources) => {
  ...
  writeFileSync(
    ...
    JSON.stringify({
      version: 3,
      file: file.slice(file.lastIndexOf('/') + 1).replace(/\.map$/, ''),
      sources,
      mappings: '',
    }),
  )
...
const runChecker = ({ maps = {}, src = CLEAN_SRC, distFiles = {} } = {}) => {
  ...
  try {
    ...
    const result = spawnSync(process.execPath, [checker, dist, '--src', srcDir], {
      encoding: 'utf8',
    })
...
test('rejects CreateMemeScreen importing a hyphenated meme-card module', () => {
  ...
  assert.match(
    result.output,
    /screens\/CreateMemeScreen\.tsx imports "\/molecules\/meme-card"|screens\/CreateMemeScreen\.tsx imports "@\/molecules\/meme-card"/,
  )
...
test('rejects LandingScreen or createMemeModel importing meme-card', () => {
  const landing = runChecker({
    ...
    src: {
      ...CLEAN_SRC,
      'screens/LandingScreen.tsx':
        "import MemeCard from '../molecules/meme-card.tsx'\nexport const LandingScreen = () => null\n",
    },
  })
...
test('exits 2 when there is nothing built to check', () => {
  const missing = spawnSync(
    process.execPath,
    [checker, join(tmpdir(), 'memeon-chunk-check-absent'), '--src', tmpdir()],
    {
      encoding: 'utf8',
    },
  )
