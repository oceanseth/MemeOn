...
function replaceTargets(proxy, target) {
  return Object.fromEntries(
    Object.entries(proxy).map(([key, options]) => [key, { ...options, target }]),
  )
}
...
try {
  ...
  const legacyPrefixProxy = {
    ...devProxyWithMarker,
    '/m': devProxyWithMarker['/m/'],
  }
  ...
  calls.length = 0
  await withDevServer(devProxyWithMarker, (origin) =>
    assertSpaAndProxies(origin, calls, /\/[@]vite\/client[\s\S]*\/src\/main\.tsx/),
  )
  ...
  calls.length = 0
  await withPreviewServer(previewProxyWithMarker, (origin) =>
    assertSpaAndProxies(
      origin,
      calls,
      new RegExp(builtEntry.replace(/[.*+?^${}()|[\]\\]/gu, '\\$&')),
    ),
  )
