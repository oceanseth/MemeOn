...
function numberFromInput(event: React.ChangeEvent<HTMLInputElement>): number {
...
export function useMemeDetailScreen(): MemeDetailScreenModel {
  ...
  const resolveHolderNames = useCallback(
    (positions: Position[], meSub: string | null) => {
      ...
      const unknown = positions.filter(
        (position) =>
          !(meSub && position.userId === meSub) && !holderNameCache.current.has(position.userId),
      )
      ...
      apiFetch<{ users: { sub: string; name: string }[] }>('/api/users?q=')
        ...
        .catch(() => {
          holderNamesRequested.current = false
        })
    },
    [send],
  )
  ...
  const load = useCallback(() => {
    ...
    apiFetch<{ meme: Meme; positions: Position[] }>(`/api/memes/${memeId}`)
      .then((result) => {
        send({ type: 'LOADED', meme: result.meme, positions: result.positions })
        resolveHolderNames(result.positions, auth.user?.sub ?? null)
      })
      .catch(() => send({ type: 'NOT_FOUND' }))
    // sources are additive; the spreading card hides when sources.length===0 — do not FAIL the page
    apiFetch<MemeStats>(`/api/memes/${memeId}/stats`)
  ...
  useMountEffect(() => {
      ...
      apiFetch<{ memes: Meme[] }>('/api/binder')
        .then((result) => {
          ...
          if (installed && current?.id === meme.id && auth.user?.sub === me.sub) {
            send({
              type: 'SET_PLEX_BINDER',
              binder: result.memes.filter((candidate) => candidate.id !== meme.id),
            })
          }
        })
    ...
    const memeId = actor.getSnapshot().context.id
    if (memeId)
      apiFetch<Memeplex>(`/api/memes/${memeId}/memeplex`)
        .then((plex) => send({ type: 'SET_PLEX', plex }))
        .catch(() => send({ type: 'SET_PLEX_ERR', err: copy.memeplex.loadFailed }))
    return () => {
      installed = false
  ...
  const act = useCallback(
    async (operation: () => Promise<unknown>, message: string, kind?: 'list' | 'buy') => {
      ...
      try {
        ...
        send({
          type: 'FAIL',
          err: error instanceof Error ? error.message : copy.errors.action,
        })
      }
    },
    [load, refresh, send],
  ...
  const buySharesChange: ChangeEventHandler<HTMLInputElement> = (event) =>
    send({ type: 'SET_BUY_SHARES', shares: numberFromInput(event) })
  const sellSharesChange: ChangeEventHandler<HTMLInputElement> = (event) =>
    send({ type: 'SET_SELL_SHARES', shares: numberFromInput(event) })
  const priceChange: ChangeEventHandler<HTMLInputElement> = (event) =>
  ...
  const onPlexAdd = (memeId: string) => {
    ...
    const linked = new Set([
      live.meme.id,
      ...(live.plex?.ancestors.map((candidate) => candidate.id) ?? []),
      ...(live.plex?.remixes.map((candidate) => candidate.id) ?? []),
      ...(live.plex?.related.map((candidate) => candidate.id) ?? []),
    ])
    if (linked.has(memeId)) {
      send({
        type: 'SET_PLEX_ERR',
        err: memeId === live.meme.id ? copy.memeplex.isThisMeme : copy.memeplex.alreadyLinked,
      })
      return
    }
    ...
    void post(`/api/memes/${live.meme.id}/memeplex`, { memeId })
      .then(() => {
        send({ type: 'SET_PLEX_MSG', msg: copy.memeplex.added })
      ...
      .catch((error) =>
        send({
          type: 'SET_PLEX_ERR',
          err: error instanceof Error ? error.message : copy.memeplex.addFailed,
        }),
      )
  }
    ...
    void act(
      () =>
        post(`/api/memes/${meme.id}/buy`, {
          shares: clampShares(actor.getSnapshot().context.buyShares, meme.listing?.shares ?? 100),
        }),
      copy.toasts.bought,
      'buy',
    )
    ...
    detail: buildMemeDetailModel({
      ...
      actions: {
        ...
        onLogin: () => {
          ...
          void beginMaskyLogin().catch((error) =>
            send({
              type: 'LOGIN_FAIL',
              err: error instanceof Error ? error.message : copy.errors.login,
            }),
          )
        },
        ...
        onToggleVisibility: () =>
          void act(
            () =>
              post(`/api/memes/${meme.id}/visibility`, {
                private: !meme.private,
              }),
            meme.private ? copy.toasts.madePublic : copy.toasts.madePrivate,
          ),
        onUnlist: () =>
          void act(() => post(`/api/memes/${meme.id}/unlist`, {}), copy.toasts.delisted),
        onList: () =>
          void act(
            () =>
              post(`/api/memes/${meme.id}/list`, {
                shares: clampShares(actor.getSnapshot().context.sellShares, Math.max(1, myShares)),
                pricePerShare: clampPrice(actor.getSnapshot().context.price),
              }),
            copy.toasts.listed,
            'list',
          ),
        ...
        onDeleteConfirm: () => {
          ...
          void apiFetch(`/api/memes/${live.id}`, { method: 'DELETE' })
            ...
            .catch((error) =>
              send({
                type: 'FAIL',
                err: error instanceof Error ? error.message : copy.errors.delete,
              }),
            )
        },
