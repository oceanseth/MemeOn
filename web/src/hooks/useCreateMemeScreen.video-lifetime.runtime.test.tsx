...
function stubPendingRemixVideo(startedAt: number, onStatus: () => Promise<Response>) {
  ...
  vi.stubGlobal(
    ...
    vi.fn<typeof fetch>((input) => {
      ...
      if (path === '/api/memes/remix-a') {
        return Promise.resolve(Response.json({ meme: sourceMeme('remix-a') }))
      }
      if (path === '/api/aigen/video/render-a') return onStatus()
...
function link(label: string): HTMLAnchorElement {
  const found = [...host.querySelectorAll<HTMLAnchorElement>('a')].find(
...
async function startVideo(kind: StartKind, prompt: string): Promise<void> {
  ...
  if (kind === 'restyle')
    await pick(copy.remix.videoStyleLabel, copy.remix.videoStyleOptions.restyle)
  const promptControl = [...host.querySelectorAll<HTMLTextAreaElement>('textarea')].find(
    (candidate) =>
      candidate.parentElement?.textContent?.includes('What to change') ||
      candidate.parentElement?.textContent?.includes('Edit prompt'),
  )!
...
describe('CreateMemeRoute video lifetime ownership', () => {
      ...
      vi.stubGlobal(
        ...
        vi.fn<typeof fetch>((input, init) => {
          ...
          if (path === '/api/memes/remix-b')
            return Promise.resolve(Response.json({ meme: sourceMeme('remix-b') }))
          if (path === '/api/aigen/image')
            return Promise.resolve(Response.json({ imageUrl: '/thumb.png' }))
          if (path === '/api/aigen/image-edit')
            return Promise.resolve(Response.json({ imageUrl: '/edited.png' }))
  ...
  it('retains a known pending record after unmount and ignores a late in-flight completion', async () => {
    ...
    stubPendingRemixVideo(Date.now(), () => {
      statusRequests += 1
  ...
  it('resumes and normally completes the pending job for the matching route', async () => {
    vi.useFakeTimers()
    stubPendingRemixVideo(Date.now(), () =>
      Promise.resolve(Response.json({ status: 'video', videoUrl: '/finished-a.mp4' })),
    )
    await renderAt('/binder/new?remix=remix-a')
    await act(async () => {
      await vi.advanceTimersByTimeAsync(5000)
    ...
    expect(
      host.querySelector<HTMLVideoElement>('video[data-slot="meme-art"]')?.getAttribute('src'),
    ).toBe('/finished-a.mp4')
  ...
  it('resumes a non-remix pending job on /binder/new', async () => {
    ...
    sessionStorage.setItem(
      PENDING_VIDEO_KEY,
      JSON.stringify({
        generationId: 'render-fresh',
        startedAt,
        imageUrl: '/pending-fresh.png',
        remixId: null,
      }),
    )
    vi.stubGlobal(
      ...
      vi.fn<typeof fetch>((input) => {
        ...
        if (path === '/api/aigen/video/render-fresh') {
          return Promise.resolve(
            Response.json({ status: 'video', videoUrl: '/finished-fresh.mp4' }),
          )
        }
        throw new Error(`Unexpected request: ${path}`)
      }),
