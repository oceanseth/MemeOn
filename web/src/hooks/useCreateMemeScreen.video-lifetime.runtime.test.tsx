...
async function startVideo(kind: StartKind, prompt: string): Promise<void> {
  ...
  if (kind === 'restyle') await pick(copy.remix.videoStyleLabel, copy.remix.videoStyle.restyle)
  const promptControl = [...host.querySelectorAll<HTMLTextAreaElement>('textarea')]
    .find((candidate) => candidate.parentElement?.textContent?.includes(copy.remix.promptLabelPrecise) || candidate.parentElement?.textContent?.includes(copy.remix.promptLabelEdit))!
