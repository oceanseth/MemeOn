...
describe('buildThemeControlModel', () => {
  it('resolves labels and the cycle sentence from sharedCopy.theme', () => {
    ...
    const model = buildThemeControlModel({ value: 'auto', onChange, variant: 'button' })

    expect(model.groupLabel).toBe(sharedCopy.theme.group)
    expect(model.cycleLabel).toBe(sharedCopy.theme.cycle(sharedCopy.theme.auto, sharedCopy.theme.light))
  ...
  it('cycles dark back to auto', () => {
    const model = buildThemeControlModel({ value: 'dark', onChange: vi.fn(), variant: 'segmented' })
    expect(model.cycleLabel).toBe(sharedCopy.theme.cycle(sharedCopy.theme.dark, sharedCopy.theme.auto))
