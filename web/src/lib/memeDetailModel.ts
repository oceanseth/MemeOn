...
export function buildMemeDetailModel({
  ...
  const listing: DetailListingModel | null = meme.listing && meme.listing.shares > 0 ? {
    ...
    buyButtonProps: {
      onClick: () => { if (buyTotal > CONFIRM_SPEND_OVER) actions.requestBuyConfirm(); else actions.runBuy() },
    ...
    unlistButtonProps: { onClick: actions.onUnlist, disabled: phase === 'listing', 'aria-busy': phase === 'listing' },
  } : null

  const detailActions: DetailActionModel[] = []
  if (user) detailActions.push({ label: copy.actions.remix, icon: 'dna', buttonProps: { onClick: actions.onRemix } })
  if (user && meme.creatorId === ARCHIVE_SUB) detailActions.push({ label: copy.actions.claim, icon: 'film', buttonProps: { onClick: actions.onStartClaim } })
  if (myShares === 100) detailActions.push({ label: meme.private ? copy.actions.makePublic : copy.actions.makePrivate, icon: meme.private ? 'globe' : 'eye-off', buttonProps: { onClick: actions.onToggleVisibility } })
  if (myShares === 100 && meme.private) detailActions.push({ label: copy.actions.delete, icon: 'trash-2', variant: 'destructive', buttonProps: { onClick: actions.onStartDelete } })
  ...
  const capTableNote: string | null = meme.listing && meme.listing.shares > 0
    ? copy.capTable.note(meme.listing.shares, isSeller
        ? null
        : holderLabel(meme.listing.sellerId, user?.sub ?? null, context.holderNames, holderNameCache))
    : null
