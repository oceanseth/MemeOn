...
export const Segmented: Story = {
  play: async ({ canvasElement }) => {
    ...
    const light = within(group).getByRole('button', {
      name: new RegExp(theme.light),
    })
    await expect(light).toHaveAttribute('aria-pressed', 'true')
    await expect(
      within(group).getByRole('button', { name: new RegExp(theme.auto) }),
...
export const HeaderButton: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const button = canvas.getByRole('button', {
      name: theme.cycle(theme.auto, theme.light),
    })
...
export const HeaderButtonPublic: Story = {
  ...
  play: async ({ canvasElement }) => {
    const button = within(canvasElement).getByRole('button', {
      name: theme.cycle(theme.dark, theme.auto),
    })
...
export const ButtonCycles: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await userEvent.click(
      canvas.getByRole('button', {
        name: theme.cycle(theme.light, theme.dark),
      }),
    )
    await expect(
      canvas
        .getByRole('button', { name: theme.cycle(theme.dark, theme.auto) })
        .querySelector('svg'),
    ).not.toBeNull()
...
export const Dark: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(
      within(group).getByRole('button', { name: new RegExp(theme.dark) }),
