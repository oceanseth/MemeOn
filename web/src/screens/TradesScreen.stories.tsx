...
const memeNames: TradeMemeInfoMap = {
  [paperMeme.id]: {
    ...
    tierKey: 'paper',
    tierName: 'Paper',
    tierLabel: copy.memeTierLabel('Paper', 'common'),
    reshares: 0,
  },
  [silverMeme.id]: {
    ...
    tierKey: 'silver',
    tierName: 'Silver',
    tierLabel: copy.memeTierLabel('Silver', 'uncommon'),
    reshares: 12,
  },
}
const closedDialog = buildConfirmDialogModel({
  open: false,
  title: '',
  message: '',
  onConfirm: noop,
...
const empty: TradesScreenModel = {
  ...
  newTradeButtonLabel: copy.newTrade,
  newTradeButtonProps: {
    onClick: noop,
    'aria-expanded': false,
    'aria-controls': 'trade-composer',
  },
  compose: null,
  open: [],
  openCountLabel: null,
...
const compose: TradeComposerModel = {
  ...
  friendSelectProps: {
    value: '',
    onValueChange: (value: string | null) => composerActions.friend(value),
  },
  friendSelectItems: [
    { value: '', label: copy.composer.pickFriend },
    { value: friendAccepted.sub, label: friendAccepted.name },
  ],
  youGiveLegend: copy.composer.youGiveLegend,
  youGiveBinderLabel: copy.composer.youGiveBinder,
  giveMemeSelectProps: {
    value: '',
    onValueChange: (value: string | null) => composerActions.giveMeme(value),
  },
  giveMemeSelectItems: [
    { value: '', label: copy.noMeme },
    {
      value: giftablePaper.id,
      label: copy.composer.binderOption(giftablePaper.title, giftablePaper.myShares ?? 0),
    },
  ],
  showGiveShares: true,
  ...
  giveSharesInputProps: {
    ...
    onChange: (event: ChangeEvent<HTMLInputElement>) =>
      composerActions.giveShares(Number(event.target.value)),
  },
  giveSharesHint: copy.composer.giveSharesHint(12),
  ...
  giveCoinsInputProps: {
    ...
    max: meLou.coins,
    onChange: (event: ChangeEvent<HTMLInputElement>) =>
      composerActions.giveCoins(Number(event.target.value)),
  },
  giveCoinsHint: copy.composer.giveCoinsHint(meLou.coins),
  youGetLegend: copy.composer.youGetLegend,
  youGetMemesLabel: copy.composer.youGetMemes,
  getMemeSelectProps: {
    value: '',
    onValueChange: (value: string | null) => composerActions.getMeme(value),
  },
  getMemeSelectItems: [
    { value: '', label: copy.noMeme },
    { value: silverMeme.id, label: silverMeme.title },
  ],
  showGetShares: true,
  ...
  getSharesInputProps: {
    ...
    max: 100,
    onChange: (event: ChangeEvent<HTMLInputElement>) =>
      composerActions.getShares(Number(event.target.value)),
  },
  ...
  getCoinsInputProps: {
    ...
    min: 0,
    onChange: (event: ChangeEvent<HTMLInputElement>) =>
      composerActions.getCoins(Number(event.target.value)),
  },
...
const pastTrade = buildTradeCardModel({
  trade: { ...proposedTrade, id: 'trade-accepted', status: 'accepted' },
  meSub: meLou.sub,
  memeNames,
  now: NOW,
})
...
export const LoadError: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(
      canvas.queryByRole('heading', { name: copy.lists.openHeading }),
...
export const Ready: Story = {
  args: {
    phase: 'ready',
    open: [openTrade],
    openCountLabel: copy.openCount(1),
    history: [pastTrade],
  },
  play: async ({ canvasElement }) => {
...
export const Proposed: Story = {
  args: {
    phase: 'ready',
    msg: 'Trade proposed. It is on their table now.',
    open: [openTrade],
    openCountLabel: copy.openCount(1),
    history: [pastTrade],
  },
...
export const Composing: Story = {
  ...
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement)
    /* the composer's two sides are field sets, the grid keeps them side by side by that slot */
    await expect(canvasElement.querySelectorAll('[data-slot="field-set"]')).toHaveLength(2)
    await expect(canvasElement.querySelector('[data-slot="field-legend"]')).toHaveAttribute(
      'data-variant',
      'legend',
    )
    const legends = [...canvasElement.querySelectorAll('[data-slot="field-legend"]')]
    await expect(legends.map((legend) => legend.textContent)).toEqual([
      copy.composer.youGiveLegend,
      copy.composer.youGetLegend,
    ])
    const selects = canvas.getAllByRole('combobox')
    await pickOption(selects[0]!, friendAccepted.name)
    await pickOption(
      selects[1]!,
      copy.composer.binderOption(giftablePaper.title, giftablePaper.myShares ?? 0),
    )
    await pickOption(selects[2]!, silverMeme.title)
    ...
    await expect(composerActions.friend).toHaveBeenCalledWith(friendAccepted.sub)
    await expect(composerActions.giveMeme).toHaveBeenCalledWith(giftablePaper.id)
    await expect(composerActions.getMeme).toHaveBeenCalledWith(silverMeme.id)
    await expect(composerActions.giveShares).toHaveBeenLastCalledWith(7)
    await expect(composerActions.giveCoins).toHaveBeenLastCalledWith(12)
    await expect(composerActions.getShares).toHaveBeenLastCalledWith(6)
    await expect(composerActions.getCoins).toHaveBeenLastCalledWith(8)
    await expect(composerActions.submit).toHaveBeenCalledOnce()
  },
}
...
export const ComposingNoFriends: Story = {
  args: {
    ...
    compose: {
      ...compose,
      noFriends: true,
      friendSelectItems: [{ value: '', label: copy.composer.pickFriend }],
    },
  },
}
...
export const ConfirmingWithdraw: Story = {
  ...
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement)
    const dialog = canvas.getByRole('alertdialog', {
      name: copy.confirm.withdrawTitle,
    })
