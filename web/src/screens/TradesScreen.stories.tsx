...
const composerActions = {
  friend: fn(), giveMeme: fn(), giveShares: fn(), giveCoins: fn(), getMeme: fn(), getShares: fn(), getCoins: fn(), propose: fn(), submit: fn(),
}
...
const compose: TradeComposerModel = {
  ...
  giveMemeSelectProps: { value: '', onValueChange: (value: string | null) => composerActions.giveMeme(value) },
  giveMemeSelectItems: [{ value: '', label: copy.noMeme }, { value: giftablePaper.id, label: copy.composer.binderOption(giftablePaper.title, giftablePaper.myShares ?? 0) }],
  showGiveShares: true,
  sharesToGiveLabel: copy.composer.sharesToGive,
  giveSharesInputProps: { value: 5, min: 1, max: 12, onChange: (event: ChangeEvent<HTMLInputElement>) => composerActions.giveShares(Number(event.target.value)) },
  giveSharesHint: copy.composer.giveSharesHint(12),
  braincellsAddLabel: copy.composer.braincellsAdd,
  giveCoinsInputProps: { value: 0, min: 0, max: meLou.coins, onChange: (event: ChangeEvent<HTMLInputElement>) => composerActions.giveCoins(Number(event.target.value)) },
  ...
  getMemeSelectProps: { value: '', onValueChange: (value: string | null) => composerActions.getMeme(value) },
  getMemeSelectItems: [{ value: '', label: copy.noMeme }, { value: silverMeme.id, label: silverMeme.title }],
  showGetShares: true,
  sharesToGetLabel: copy.composer.sharesToGet,
  getSharesInputProps: { value: 5, min: 1, max: 100, onChange: (event: ChangeEvent<HTMLInputElement>) => composerActions.getShares(Number(event.target.value)) },
  braincellsGetLabel: copy.composer.braincellsGet,
  getCoinsInputProps: { value: 0, min: 0, onChange: (event: ChangeEvent<HTMLInputElement>) => composerActions.getCoins(Number(event.target.value)) },
...
export const Composing: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(canvasElement.querySelector('[data-slot="field-legend"]')).toHaveAttribute('data-variant', 'legend')
    ...
    await pickOption(selects[1]!, copy.composer.binderOption(giftablePaper.title, giftablePaper.myShares ?? 0))
    await pickOption(selects[2]!, silverMeme.title)
    const inputs = canvas.getAllByRole('spinbutton')
    for (const [input, value] of [[inputs[0], '7'], [inputs[1], '12'], [inputs[2], '6'], [inputs[3], '8']] as const) {
