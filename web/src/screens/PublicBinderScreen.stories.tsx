...
const handlers = {
  tabsProps: { value: 'binder' as const, onValueChange: fn() },
...
const publicBinder: ProfileScreenModel = {
  ...
  documentTitle: profileCopy.documentTitleBinder,
  ...
  showEmpty: false,
  emptyTitle: profileCopy.empty.other.created.title(palProfile.name),
...
export const PublicBinder: Story = {
  play: async ({ canvasElement }) => {
    ...
    await expect(canvas.getByRole('heading', { name: profileCopy.hero.binderTitle(palProfile.name) })).toBeInTheDocument()
    await expect(canvasElement.querySelector('[data-slot="public-binder-hero"]')).toBeInTheDocument()
    await expect(canvasElement.querySelector('[data-slot="profile-identity"]')).toBeNull()
  },
}
