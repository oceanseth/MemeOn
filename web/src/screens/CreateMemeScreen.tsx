...
/** One source chip. The engine names the state and the label; the chrome for that state lives here. */
function ModeChip({
  model,
  icon,
}: {
...
export function CreateMemeScreen({
  ...
  const remixMode = getModeButtonProps('remix')
  const generateMode = getModeButtonProps('generate')
  const videoMode = getModeButtonProps('video')
  const uploadMode = getModeButtonProps('upload')
  const giphyMode = getModeButtonProps('giphy')
  const urlMode = getModeButtonProps('url')
  if (showSuccess) {
    return (
  ...
  return (
    <PageContainer as="main" id="main" tabIndex={-1}>
      ...
      <div data-slot="mint-modes" className={MODE_ROW} {...modeGroupProps}>
        {showRemixModeButton && <ModeChip model={remixMode} icon="dna" />}
        <ModeChip model={generateMode} icon="palette" />
        <ModeChip model={videoMode} icon="clapperboard" />
        <ModeChip model={uploadMode} icon="upload" />
        <ModeChip model={giphyMode} icon="film" />
        <ModeChip model={urlMode} icon="link" />
      </div>
        ...
        <Card>
          ...
          <div data-slot="form-grid" className={cn(FORM_GRID, 'mt-5')} {...formProps}>
            ...
            {showRemixPanel ? (
              <>
                {remixSource ? (
                  <Toolbar>
                    ...
                    <Hint as="span">
                      {remixSourcePrefix}{' '}
                      <Link {...remixSource.linkProps}>"{remixSource.title}"</Link> {remixSourceBy}{' '}
                ...
                {showVideoRemixStyle && (
                  <Field>
                    <FieldLabel>{remixVideoStyleLabel}</FieldLabel>
                    <Select aria-label={remixVideoStyleLabel} {...videoModeSelectProps} />
                  </Field>
                )}
                <Field>
                  ...
                  <div data-slot="approval-card">
                    <Empty variant="success" size="inline" role="none">
                      <EmptyHeader>
                        ...
                        <EmptyDescription>{remixApprovalBody}</EmptyDescription>
                      </EmptyHeader>
                      <Field className="w-full">
                        <FieldLabel>{remixMotionLabel}</FieldLabel>
                        <Textarea {...motionPromptTextareaProps} rows={3} />
                      </Field>
                      <Toolbar>
                ...
                {showGiphyPick && giphyPick && (
                  <>
                    <Hint className="mb-2">
                      ...
                      {giphyPick.authorLabel} {giphySelectedSuffix}
                    </Hint>
                    <Field>
                      <FieldLabel>{giphyRemixPromptLabel}</FieldLabel>
                      <Textarea {...giphyPromptTextareaProps} rows={2} />
                    </Field>
                    {showGiphyRemixButton && (
                      <Toolbar className="mt-1">
              ...
              <>
                <Field>
                  <FieldLabel>{urlLabel}</FieldLabel>
                ...
                <Field>
                  <FieldLabel>{urlRemixPromptLabel}</FieldLabel>
                  <Textarea {...urlPromptTextareaProps} rows={2} />
                </Field>
                {showUrlApplyEdit && (
                  <Toolbar className="mt-1">
                    <Button variant="primary" {...applyUrlEditButtonProps}>
                      <Icon name="sparkles" size={16} /> {urlApplyEditLabel}
                    </Button>
                    <span className={COST_NOTE}>{creditsNote}</span>
                  </Toolbar>
                )}
              ...
              <>
                <Field>
                  <FieldLabel>{generatePromptLabel}</FieldLabel>
                  <Textarea
                    {...generatePromptTextareaProps}
                    rows={3}
              ...
              {showBusy && (
                ...
                <div data-slot="busy-card">
                  <Empty variant="busy" size="inline" role="none">
                    ...
                    <EmptyDescription>{busyStayText}</EmptyDescription>
                  </Empty>
                </div>
              )}
            </LiveRegion>
            ...
            <p className={FORM_NOTE}>{formFooter}</p>
          </div>
        </Card>
          ...
          <Card className="flex flex-col">
            ...
            <div className="mt-4 flex items-center justify-between gap-3 max-md:flex-col max-md:items-stretch max-md:gap-2">
              ...
              <Button variant="primary" className="max-md:w-full" {...mintButtonProps}>
                <Icon name="sparkles" size={16} /> {mintButtonLabel}
              </Button>
            </div>
          </Card>
        </div>
