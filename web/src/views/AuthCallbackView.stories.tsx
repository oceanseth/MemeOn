...
function maskyExchange(_request: unknown, scenario: { user: typeof meLou | null }) {
  ...
  return {
    body: {
      ...
      firebaseToken: null,
      profile: { sub: meLou.sub, name: meLou.name, picture: meLou.picture, coins: meLou.coins },
    },
  }
}

async function seedInviteAfterClear(context: Parameters<typeof connectedBeforeEach>[0], extra?: () => void) {
...
export const InviteAcceptFailedStaysOnCallback: Story = {
  ...
  loaders: [connectedLoader({
    ...
    overrides: {
      ...
      'POST /api/invites/accept': (_request, scenario) => {
        const attempts = scenario.requests.filter((request) => request.path === '/api/invites/accept').length
  ...
  play: async ({ canvasElement, loaded }) => {
    const canvas = within(canvasElement)
    await expect(await canvas.findByRole('heading', { name: authStatusCopy.callback.inviteFailed.title })).toBeInTheDocument()
    ...
    await expect(canvas.getByRole('status', { name: 'Current route' })).toHaveTextContent('/auth/callback')
    ...
    const firstAccept = loaded.scenario.requests.filter((request: { path: string }) => request.path === '/api/invites/accept')
    ...
    await expect(loaded.scenario.authorizationNavigations).toEqual([])
    await waitFor(() => expect(canvas.getByRole('status', { name: 'Current route' })).toHaveTextContent('/friends'))
    await expect(canvas.getByText('Friends')).toBeInTheDocument()
    const accepts = loaded.scenario.requests.filter((request: { path: string }) => request.path === '/api/invites/accept')
    await expect(accepts).toHaveLength(2)
    await expect(accepts.map((request: { body: unknown }) => request.body)).toEqual([{ inviterId: INVITER }, { inviterId: INVITER }])
    await expect(loaded.scenario.authorizationNavigations).toEqual([])
  },
}
...
export const InviteAcceptSucceedsToFriends: Story = {
  ...
  play: async ({ canvasElement, loaded }) => {
    ...
    await waitFor(() => expect(canvas.getByRole('status', { name: 'Current route' })).toHaveTextContent('/friends'))
    await expect(canvas.getByText('Friends')).toBeInTheDocument()
    await expect(canvas.queryByRole('heading', { name: authStatusCopy.callback.inviteFailed.title })).not.toBeInTheDocument()
    await expect(loaded.scenario.requests.find((request: { path: string }) => request.path === '/api/invites/accept')?.body).toEqual({ inviterId: INVITER })
  },
}
...
export const InviteAcceptSucceedsToPostLogin: Story = {
  ...
  play: async ({ canvasElement, loaded }) => {
    const canvas = within(canvasElement)
    await waitFor(() => expect(canvas.getByRole('status', { name: 'Current route' })).toHaveTextContent('/discord/link'))
