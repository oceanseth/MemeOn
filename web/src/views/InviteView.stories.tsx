...
export const ReadyAndAccept: Story = {
  ...
  play: async ({ canvasElement, loaded }) => {
    ...
    await expect(
      await canvas.findByRole('heading', { name: /pal invited you/ }),
    ).toBeInTheDocument()
...
export const FatalLoadError: Story = {
  loaders: [
    connectedLoader({
      failures: {
        'GET /api/invite/user-pal': { error: 'expired', status: 410 },
      },
    }),
  ],
  beforeEach: async (context) => connectedBeforeEach(context),
  ...
  play: async ({ canvasElement }) => {
    await expect(await within(canvasElement).findByText(/invalid or expired/)).toBeInTheDocument()
  },
}
...
export const LoadingThenReady: Story = {
  loaders: [
    connectedLoader({
      overrides: {
        'GET /api/invite/user-pal': async (_request, scenario) => {
          await scenario.waitForRelease('invite')
          return { body: scenario.inviteData.get('user-pal') }
        },
      },
    }),
  ],
  ...
  play: async ({ canvasElement, loaded }) => {
    ...
    loaded.scenario.release('invite')
    await expect(
      await canvas.findByRole('heading', { name: /pal invited you/ }),
    ).toBeInTheDocument()
  },
...
export const AcceptFailureReturnsToReady: Story = {
  ...
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement)
    await userEvent.click(await canvas.findByRole('button', { name: /Accept & befriend pal/ }))
    await expect(await canvas.findByRole('alert')).toHaveTextContent(/Couldn't accept this invite/)
    await expect(canvas.getByRole('button', { name: /Accept & befriend pal/ })).not.toHaveAttribute(
      'aria-disabled',
    )
  },
}
