    ...
    await expect(
      loaded.scenario.requests.find(
        (request: { path: string; method: string }) =>
          request.path === `/api/memes/${listedHolo.id}/memeplex` && request.method === 'POST',
      )?.body,
    ).toEqual({ memeId: 'https://memeon.ai/m/%E0%A4%A' })
  },
}
...
export const DeleteCancelAndConfirm: Story = {
  loaders: [
    connectedLoader({
      overrides: {
        [`GET /api/memes/${listedHolo.id}`]: () => ({
          body: {
            meme: { ...listedHolo, listing: null, private: true },
  ...
  play: async ({ canvasElement, loaded }) => {
    ...
    await waitFor(() =>
      expect(
        loaded.scenario.requests.some(
          (request: { path: string; method: string }) =>
            request.path === `/api/memes/${listedHolo.id}` && request.method === 'DELETE',
        ),
      ).toBe(true),
    )
  },
...
export const BuyerFlow: Story = {
  ...
  play: async ({ canvasElement, loaded }) => {
    ...
    await expect(
      loaded.scenario.requests.find((request: { path: string }) => request.path.endsWith('/buy'))
        ?.body,
    ).toEqual({ shares: 2 })
  },
}
...
export const MissingIsEmpty: Story = {
  loaders: [
    connectedLoader({
      failures: {
        [`GET /api/memes/${listedHolo.id}`]: { error: 'missing', status: 404 },
      },
    }),
  ],
  beforeEach: async (context) => connectedBeforeEach(context),
  ...
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement)
    await expect(
      await canvas.findByText(/may have been deleted or made private/),
    ).toBeInTheDocument()
...
export const HolderNameCacheIsMountLocal: Story = {
  loaders: [
    connectedLoader({
      overrides: {
        [`GET /api/memes/${listedHolo.id}`]: (_request, scenario) => ({
          body: {
            meme: scenario.memes.find((candidate) => candidate.id === listedHolo.id),
            positions: [{ userId: 'user-a', shares: 25 }],
          },
        }),
        'GET /api/users': (_request, scenario) => ({
          body: { users: [scenario.profiles.get('user-a')!.profile] },
        }),
      },
    }),
  ],
  ...
  render: (_args, { loaded }) => (
    <ConnectedStory scenario={loaded.scenario}>
      <RemountStory>{(key) => <MemeDetailView key={key} />}</RemountStory>
    </ConnectedStory>
  ),
  play: async ({ canvasElement, loaded }) => {
    const canvas = within(canvasElement)
    ...
    await expect(
      loaded.scenario.requests.filter(
        (request: { path: string }) => request.path === '/api/users?q=',
      ),
    ).toHaveLength(2)
  },
}
...
export const LoadingThenReady: Story = {
  loaders: [
    connectedLoader({
      overrides: {
        [`GET /api/memes/${listedHolo.id}`]: async (_request, scenario) => {
          await scenario.waitForRelease('detail')
          return {
            body: {
              meme: scenario.memes.find((candidate) => candidate.id === listedHolo.id),
              positions: [{ userId: meLou.sub, shares: 100 }],
  ...
  play: async ({ canvasElement, loaded }) => {
    ...
    loaded.scenario.release('detail')
    await expect(await canvas.findByRole('heading', { name: listedHolo.title })).toBeInTheDocument()
  },
}
...
export const ListShares: Story = {
  ...
  play: async ({ canvasElement, loaded }) => {
    const canvas = within(canvasElement)
    const inputs = await canvas.findAllByRole('spinbutton')
    await userEvent.clear(inputs[0]!)
    await userEvent.type(inputs[0]!, '4')
    await userEvent.clear(inputs[1]!)
    await userEvent.type(inputs[1]!, '2')
    await userEvent.click(canvas.getByRole('button', { name: 'List' }))
    await expect(await canvas.findByText('Listed on the marketplace')).toBeInTheDocument()
    await expect(
      loaded.scenario.requests.find((request: { path: string }) => request.path.endsWith('/list'))
        ?.body,
    ).toEqual({ shares: 4, pricePerShare: 2 })
  },
}
...
export const BuyFailureKeepsLoadedDetail: Story = {
  loaders: [
    connectedLoader({
      user: { ...meLou, sub: 'buyer', name: 'buyer' },
      failures: {
        [`POST /api/memes/${listedHolo.id}/buy`]: {
          error: 'insufficient braincells',
          status: 409,
        },
      },
    }),
  ...
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement)
    await userEvent.click(await canvas.findByRole('button', { name: /Buy for/ }))
    await expect(await canvas.findByText('insufficient braincells')).toBeInTheDocument()
    await expect(canvas.getByRole('heading', { name: listedHolo.title })).toBeInTheDocument()
  },
}
