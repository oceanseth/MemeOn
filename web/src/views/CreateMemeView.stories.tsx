...
export const GenerateAndMint: Story = {
  ...
  play: async ({ canvasElement, loaded }) => {
    ...
    await userEvent.type(
      canvas.getByRole('textbox', { name: copy.form.titleLabel }),
      'generated story',
    )
    await userEvent.type(
      canvas.getByRole('textbox', { name: copy.form.promptLabel }),
      'a cat on the moon',
    )
    await userEvent.click(canvas.getByRole('button', { name: copy.generate.renderImage }))
    await expect(await canvas.findByRole('img', { name: /^Preview of/ })).toHaveAttribute(
      'src',
      loaded.scenario.generatedImage,
    )
    ...
    await waitFor(() =>
      expect(
        loaded.scenario.requests.some(
          (request: { method: string; path: string; body: { title?: string } }) =>
            request.method === 'POST' &&
            request.path === '/api/memes' &&
            request.body.title === 'generated story',
        ),
      ).toBe(true),
    )
    ...
    await expect(canvas.getByRole('textbox', { name: copy.form.success.shareLink })).toHaveValue(
      `${window.location.origin}/m/meme-minted`,
    )
...
export const GiphyKeyboardSearchPickAndMint: Story = {
  ...
  play: async ({ canvasElement, loaded }) => {
    ...
    const query = canvas.getByRole('searchbox', {
      ...
      expect(
        loaded.scenario.requests.filter(
          (request: { method: string; path: string }) =>
            request.method === 'POST' && request.path === '/api/memes',
        ),
      ).toHaveLength(1),
    )
    const mint = loaded.scenario.requests.find(
      (request: { method: string; path: string }) =>
        request.method === 'POST' && request.path === '/api/memes',
    )
    await expect(mint?.body).toMatchObject({
      title: 'giphy story',
      source: { provider: 'giphy', id: giphyCat.id },
    })
  },
}
  ...
  play: async ({ canvasElement, loaded }) => {
    ...
    await waitFor(() =>
      expect(
        loaded.scenario.requests.some(
          (request: { method: string; path: string }) =>
            request.method === 'PUT' && request.path === '/story',
        ),
      ).toBe(true),
    )
  },
...
export const GenerationFailure: Story = {
  loaders: [
    connectedLoader({
      failures: {
        'POST /api/aigen/image': { error: 'credits exhausted', status: 402 },
      },
    }),
  ],
  beforeEach: async (context) => connectedBeforeEach(context),
