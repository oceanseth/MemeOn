...
export const LinkExpired: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(canvas.queryByRole('button', { name: copy.tryAgain })).not.toBeInTheDocument()
    await expect(canvas.getByRole('link', { name: copy.backToBrand })).toBeInTheDocument()
  },
}
...
export const UnreachableThenRetried: Story = {
  ...
  play: async ({ canvasElement, loaded }) => {
    ...
    await userEvent.click(await canvas.findByRole('button', { name: copy.tryAgain }))
