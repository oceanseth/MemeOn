...
it('keeps the legacy mint draft mounted while a post-pack refresh replaces an older account read', async () => {
    ...
    vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.pathname === '/api/me') {
        ...
        return await new Promise<Response>((resolve) =>
          held.push({ snapshot, resolve, signal: init?.signal ?? null }),
        )
      }
  ...
  for (
    let attempt = 0;
    attempt < 100 && !host.querySelector('[data-slot="quest-claim"]');
    attempt += 1
  ) {
    await act(async () => {
      await new Promise((r) => setTimeout(r, 10))
  ...
  await act(async () => {
    ;[...host.querySelectorAll<HTMLButtonElement>('button')]
      .find((button) => button.textContent?.includes('Starter pack'))!
      .click()
    await nextTick()
    await nextTick()
  })
  ...
  expect(
    host.querySelector('[data-slot="quest-trigger"], [data-slot="braincells"]')?.textContent,
  ).toContain('52')
  expect(host.querySelector<HTMLInputElement>('input#create-title')).toBe(title)
  expect(title.value).toBe('Keep my draft')
  expect(prompt.value).toBe('Draft prompt survives account refresh')
