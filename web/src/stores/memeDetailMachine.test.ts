...
function readyActor() {
  const actor = createActor(memeDetailMachine, {
    input: { id: meme.id },
  }).start()
...
describe('memeDetailMachine reloads during actions', () => {
  ...
  it('keeps reload failures on the active action cleanup path', () => {
    for (const action of [
      ...
      if (action.phase === 'deleting')
        actor.send({ type: 'SET_CONFIRMING_DELETE', confirming: true })
      actor.send(action.event)
      actor.send({
        type: 'LOADED',
        meme: reloadedMeme,
  ...
  it('clamps money-adjacent inputs where the state lives', () => {
    const listed = {
      ...meme,
      listing: { sellerId: 'owner-a', pricePerShare: 4, shares: 10 },
    }
    const actor = createActor(memeDetailMachine, {
      input: { id: meme.id },
    }).start()
  ...
  it('keeps the memeplex notice and error channels exclusive', () => {
    ...
    expect(actor.getSnapshot().context).toMatchObject({
      plexMsg: null,
      plexErr: 'Already in the memeplex.',
    })
    actor.stop()
  })
  ...
  it('resolves every holder name in one batch', () => {
    const actor = readyActor()
    actor.send({
      type: 'SET_HOLDER_NAMES',
      names: { 'holder-b': 'Pal', 'holder-c': 'Bud' },
    })
    actor.send({ type: 'SET_HOLDER_NAMES', names: { 'holder-d': 'Chum' } })
    expect(actor.getSnapshot().context.holderNames).toEqual({
      'holder-b': 'Pal',
      'holder-c': 'Bud',
      'holder-d': 'Chum',
    })
