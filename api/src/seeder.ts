...
export async function runGiphySeed(opts: { max?: number; inventoryTarget?: number }): Promise<{
  seeded: number
  inventory: number
  skipped: boolean
  termsSwept: number
  ...
  for (const gif of await trending(50, trendingOffset).catch(() => [])) {
    ...
    if (await mintArchiveGif(gif, 'trending')) seeded++
  }
  ...
  for (let i = 0; i < TERMS_PER_RUN && seeded < room; i++) {
    ...
    for (const gif of results) {
      ...
      if (await mintArchiveGif(gif, term)) {
        fromThisTerm++
        seeded++
      }
    }
  ...
  return {
    seeded,
    inventory: alreadySeeded + seeded,
    skipped: false,
    termsSwept,
  }
}
