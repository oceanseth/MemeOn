...
| Change React UI | Read [web/src/Anatomy.mdx](web/src/Anatomy.mdx) first — its **Making a change** table says which file a copy / style / state / route / token change lands in. Tiers `atoms/ → molecules/ → organisms/ → screens/ → views/` are pure props → markup; state lives in `hooks/` + `stores/`; prop-bag builders in `lib/*Model.ts`; tokens in `web/src/index.css` (`@theme static`, register new namespaces in `lib/cn.ts`). Every component has a story. Gate: `pnpm run check` (or `pnpm --filter web run check-tiers` for the structure alone). | lou 2026-09-12 |
| Change UI copy (title, caption, label, error string) | Edit **`web/src/copy/<surface>.ts`** only; builders, unit tests and story `play`s read the same constant. Surfaces not yet moved are listed in `web/scripts/copy-baseline.json` — move the strings into `copy/` first, then `pnpm --filter web run check-copy -- --update`. The ratchet fails a PR that spells new copy in `hooks/` or `lib/*Model.ts`. | lou 2026-09-13 (mo-1f9) |
| Spacing / width / radius in React UI | No arbitrary pixel utilities (`h-[46px]`, `px-[18px]`, `rounded-[17px]`). Role tokens in `web/src/index.css` `@theme static` (`--spacing-control/-control-x/-control-gap/-control-sm/-field/-icon/-gutter/-chip-x/-nav-item/-nav-gap/-avatar-hero`, `--radius-band/-well/-segment/-control-sm/-avatar-hero`, `--container-page-narrow/-card/-hero-video/-measure/-measure-sm`) read as `h-control`, `px-control-x`, `p-gutter`, `max-w-measure`…; anything else is a grid step (`p-5.5`). New names also go in `web/src/lib/cn.ts`. `[…]` only for `calc()` / `env()` / `%` / `ch`. | mo-27u 2026-09-13 |
