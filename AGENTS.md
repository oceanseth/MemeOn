...
| lucide-react / lucide | Never install. Workspace override sets the package to `-`. Popmelt toolbar icons resolve through `web/src/popmeltLucideStub.ts` via the Vite alias. `pnpm --filter web run test:shadcn-postadd` fails a package.json or lockfile that names it. | lou 2026-09-20 |
