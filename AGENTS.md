...
| Duplicate code (DRY) gate | `pnpm run check:dup` — jscpd over `api`, `web`, `shared`, `mobile/src`, `discord`. Scope, ignores (stories are excluded by design) and the fail threshold live in `.jscpd.json`; CI runs it as the `duplicate code (jscpd)` job in `web-check.yml`. | lou 2026-09-20 |
