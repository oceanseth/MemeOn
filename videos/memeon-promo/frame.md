...
# Role note: broadside's key NAMES are kept (atoms are sacred), but the generator
# mis-detected this dark preset as light and inverted it. Repainted by hand onto
# MemeOn's real CSS variables so `ink-black` is the DARK ground and `cream` is the
# LIGHT text — the roles the key names actually describe.
colors:
  ink-black: "#0B0D14"      # --bg, the app canvas
  ink-black-alt: "#131722"  # --bg-raised
  fire-orange: "#7FD4FF"    # --accent, holo cyan — the sole chrome accent
  cream: "#E8ECF4"          # --text
  cream-muted: "#97A0B5"    # --text-dim
  cream-hint: "#5B6478"
  border-dark: "#262D40"    # --border
  ink-on-orange-muted: "rgba(11,13,20,0.75)"
  ink-on-orange-hint: "rgba(11,13,20,0.55)"
  ink-on-orange-faint: "rgba(11,13,20,0.40)"
  ink-on-orange-border: "rgba(11,13,20,0.20)"
...
- **Role correction applied by hand.** The generator mis-detected this dark preset as light and inverted it. The frontmatter has been repainted so `ink-black` = `#0B0D14` (MemeOn's `--bg`, the DARK ground) and `cream` = `#E8ECF4` (`--text`, the LIGHT type). Read every prose mention of "dark register" as a near-black ground with pale type — broadside's original intent, and MemeOn's actual app.

### The tier ladder is CONTENT, not chrome (the one sanctioned polychrome)

Broadside's "fire-orange is the only color" law governs **chrome** — kickers, rules, stat accents,
bullets. It still holds absolutely there.

The seven **virality tier colors are product data**, quoted verbatim from `shared/tiers.ts`, and are
the single sanctioned exception. They appear ONLY as the foil identity of a card (frame art, its
glow, its tier name) — never as chrome, never as a background, never as a second accent on a
non-card element. Because the chrome stays monochrome cyan, this ladder is the only polychrome
event in the film, which is exactly what makes the climb land:

| Tier | Rarity | Reshares | Color |
| --- | --- | --- | --- |
| Paper | Common | 0+ | `#a8b0bd` |
| Silver | Uncommon | 10+ | `#c8d3e0` |
| Holo | Rare | 50+ | `#7fd4ff` |
| Chrome | Ultra Rare | 250+ | `#b8c6ff` |
| Gold | Legendary | 1,000+ | `#ffd76a` |
| Prismatic | Secret Rare | 5,000+ | `#ff9af5` |
| Shiny | Mythic Shiny | 25,000+ | `#9fffe0` |

Never invent a tier, threshold, or hex. The real frame art ships in `assets/` as
`<tier>-frame.png` — composite the meme into the frame's window rather than drawing foil by hand.
